...
- v0.13.0 起，错题录入和苏格拉底训练支持浏览器语音输入：语音会先转成文字草稿，提交前可以手动修改；不保存原始录音文件。
...
3. 如果需要口述题干、答案或备注，可以在录入页使用 `语音填入位置` 和 `开始语音输入`。
4. 进入 `苏格拉底训练`，选择刚录入的薄弱点和一个模板，第一次不用修改提示词。
5. 回答时可以使用 `回答语音输入` 生成草稿，再修改并点击 `提交文本`。
6. 如果还没有配置 API Key，训练页会提示配置；其它页面仍可正常记录和复盘。
7. 训练后进入 `周度/月度复盘` 查看薄弱点和下一步建议。

## 语音输入

语音输入使用浏览器的 Web Speech API，推荐 Chrome 或 Edge。使用时需要允许浏览器访问麦克风。

- 录入页：先选择语音要填入 `题干`、`参考答案` 或 `备注`，再开始语音输入。
- 训练页：语音会进入 `语音/文字回答草稿`，你可以修改后再提交。
- 原始录音不会保存到项目目录；只有你最终保存或提交的文字会进入数据库和溯源记录。

如果浏览器不支持语音识别、麦克风权限被拒绝，或识别效果不稳定，直接使用文字输入即可。
